...
function hncInitSlider() {
    ...
    for (let i = sliders.length; 0 < i ; i--) {
        ...
        hncUpdateTrackBackground(sliders[i - 1]);
...
// slider 값 설정시 배경색 변경
function onInput_HncSlider(e) {
   // this : 이벤트를 발생시킨 range
   hncUpdateTrackBackground(this);
}

// slider value에 따라 배경색 설정
function hncUpdateTrackBackground(slider) {
    if (slider == undefined) {
        return;
    }
    let min = slider.min;
